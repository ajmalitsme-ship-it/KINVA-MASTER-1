...
<title>KiraFx Bot — Admin Dashboard</title>
<meta http-equiv="refresh" content="30">
<style>
*{margin:0;padding:0;box-sizing:border-box;}
body{font-family:'Segoe UI',Tahoma,sans-serif;background:linear-gradient(135deg,#0f0c29,#302b63,#24243e);min-height:100vh;color:#fff;}
a{color:#4facfe;text-decoration:none;}
.header{background:rgba(0,0,0,0.4);padding:18px 30px;display:flex;align-items:center;justify-content:space-between;border-bottom:1px solid rgba(255,255,255,0.1);flex-wrap:wrap;gap:10px;}
.header-left h1{font-size:1.6em;background:linear-gradient(90deg,#f093fb,#f5576c,#4facfe,#00f2fe);-webkit-background-clip:text;-webkit-text-fill-color:transparent;background-clip:text;}
.header-left p{color:#aaa;font-size:0.85em;margin-top:3px;}
.badge{display:inline-block;background:#00c853;color:#fff;padding:3px 12px;border-radius:20px;font-size:0.78em;margin-left:8px;animation:pulse 2s infinite;}
@keyframes pulse{0%,100%{opacity:1}50%{opacity:.65}}
.header-right{display:flex;gap:10px;align-items:center;}
.header-btn{background:rgba(79,172,254,0.2);border:1px solid rgba(79,172,254,0.4);color:#4facfe;padding:7px 16px;border-radius:8px;font-size:0.82em;cursor:pointer;}
/* TABS */
.tabs{display:flex;gap:0;padding:0 30px;background:rgba(0,0,0,0.3);border-bottom:1px solid rgba(255,255,255,0.08);flex-wrap:wrap;}
.tab{padding:13px 22px;cursor:pointer;font-size:0.9em;color:#aaa;border-bottom:3px solid transparent;transition:all 0.2s;}
.tab:hover{color:#fff;}
...
/* GRID */
.stats-grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(190px,1fr));gap:18px;margin-bottom:30px;}
.stat-card{background:rgba(255,255,255,0.07);border-radius:14px;padding:22px;border:1px solid rgba(255,255,255,0.1);transition:transform 0.2s,box-shadow 0.2s;position:relative;overflow:hidden;}
.stat-card::before{content:'';position:absolute;top:0;left:0;right:0;height:3px;}
.stat-card.blue::before{background:linear-gradient(90deg,#4facfe,#00f2fe);}
.stat-card.purple::before{background:linear-gradient(90deg,#f093fb,#f5576c);}
...
.stat-card.cyan::before{background:linear-gradient(90deg,#0fd850,#f9f047);}
.stat-card.red::before{background:linear-gradient(90deg,#f5576c,#f093fb);}
.stat-card:hover{transform:translateY(-4px);box-shadow:0 12px 30px rgba(0,0,0,0.4);}
.stat-icon{font-size:2.2em;margin-bottom:8px;}
.stat-value{font-size:2.2em;font-weight:800;}
.stat-card.blue .stat-value{color:#4facfe;}
.stat-card.purple .stat-value{color:#f093fb;}
.stat-card.green .stat-value{color:#43e97b;}
.stat-card.orange .stat-value{color:#fee140;}
.stat-card.cyan .stat-value{color:#0fd850;}
.stat-card.red .stat-value{color:#f5576c;}
.stat-label{color:#aaa;font-size:0.88em;margin-top:4px;}
/* SECTION */
.section-title{font-size:1.15em;color:#f093fb;margin-bottom:15px;padding-left:12px;border-left:3px solid #f093fb;}
/* TABLE */
.info-table{width:100%;border-collapse:collapse;background:rgba(255,255,255,0.04);border-radius:12px;overflow:hidden;}
.info-table tr{border-bottom:1px solid rgba(255,255,255,0.05);}
.info-table tr:last-child{border-bottom:none;}
.info-table td{padding:11px 18px;font-size:0.92em;}
.info-table td:first-child{color:#aaa;width:35%;}
...
.tool-badge{background:rgba(79,172,254,0.12);border:1px solid rgba(79,172,254,0.28);border-radius:8px;padding:9px 14px;text-align:center;font-size:0.85em;color:#4facfe;}
/* STATUS */
.status-dot{display:inline-block;width:9px;height:9px;border-radius:50%;background:#00c853;margin-right:7px;animation:pulse 2s infinite;}
/* BAR CHART */
.bar-chart{margin-top:10px;}
.bar-row{display:flex;align-items:center;gap:12px;margin-bottom:8px;}
.bar-label{color:#aaa;font-size:0.82em;width:80px;flex-shrink:0;}
.bar-track{flex:1;background:rgba(255,255,255,0.08);border-radius:4px;height:14px;overflow:hidden;}
.bar-fill{height:100%;border-radius:4px;background:linear-gradient(90deg,#4facfe,#00f2fe);transition:width 0.8s;}
.bar-val{color:#fff;font-size:0.82em;width:40px;text-align:right;flex-shrink:0;}
/* USERS TABLE */
.user-table{width:100%;border-collapse:collapse;font-size:0.88em;}
.user-table th{text-align:left;padding:10px 14px;color:#aaa;border-bottom:1px solid rgba(255,255,255,0.1);font-weight:500;}
.user-table td{padding:10px 14px;border-bottom:1px solid rgba(255,255,255,0.05);}
.user-table tr:hover td{background:rgba(255,255,255,0.04);}
.badge-prem{display:inline-block;background:linear-gradient(90deg,#f093fb,#f5576c);color:#fff;padding:2px 8px;border-radius:10px;font-size:0.78em;}
.badge-free{display:inline-block;background:rgba(255,255,255,0.15);color:#ccc;padding:2px 8px;border-radius:10px;font-size:0.78em;}
.badge-ban{display:inline-block;background:#f5576c;color:#fff;padding:2px 8px;border-radius:10px;font-size:0.78em;}
/* QUICK ACTIONS */
.action-grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(200px,1fr));gap:14px;}
.action-card{background:rgba(255,255,255,0.06);border:1px solid rgba(255,255,255,0.1);border-radius:12px;padding:18px;cursor:pointer;transition:all 0.2s;}
.action-card:hover{background:rgba(255,255,255,0.1);transform:translateY(-2px);}
.action-card .icon{font-size:1.8em;margin-bottom:8px;}
.action-card .name{font-weight:600;font-size:0.95em;}
.action-card .desc{color:#aaa;font-size:0.82em;margin-top:3px;}
/* ALERTS */
.alert{background:rgba(0,200,83,0.12);border:1px solid rgba(0,200,83,0.3);border-radius:10px;padding:12px 18px;font-size:0.9em;color:#43e97b;margin-bottom:20px;}
/* FOOTER */
.footer{text-align:center;padding:25px;color:#555;font-size:0.82em;border-top:1px solid rgba(255,255,255,0.06);margin-top:20px;}
</style>
</head>
<body>
...
<div class="header">
  <div class="header-left">
    ...
    <p>Made by Kira Version Team &nbsp;·&nbsp; v7.0 Ultra Premium &nbsp;·&nbsp; Auto-refresh every 30s</p>
  </div>
  <div class="header-right">
    <span style="color:#aaa;font-size:0.82em;">Last update: {{ now }}</span>
  </div>
</div>
...
<div class="tabs">
  ...
  <div class="tab" onclick="showTab('daily')">📈 Daily Stats</div>
...
<div class="tab-content active" id="tab-overview">
  <div class="stats-grid">
    ...
    <div class="stat-card purple"><div class="stat-icon">⭐</div><div class="stat-value">{{ stats.premium_users }}</div><div class="stat-label">Premium Users</div></div>
    <div class="stat-card green"><div class="stat-icon">✏️</div><div class="stat-value">{{ stats.total_edits }}</div><div class="stat-label">Total Edits</div></div>
    <div class="stat-card orange"><div class="stat-icon">📅</div><div class="stat-value">{{ stats.today_edits }}</div><div class="stat-label">Today's Edits</div></div>
    <div class="stat-card cyan"><div class="stat-icon">📝</div><div class="stat-value">{{ stats.total_renames }}</div><div class="stat-label">Files Renamed</div></div>
    <div class="stat-card red"><div class="stat-icon">🚫</div><div class="stat-value">{{ stats.banned_users }}</div><div class="stat-label">Banned Users</div></div>
    <div class="stat-card blue"><div class="stat-icon">🎯</div><div class="stat-value">{{ stats.total_commands }}</div><div class="stat-label">Commands Used</div></div>
    <div class="stat-card green"><div class="stat-icon">⏳</div><div class="stat-value">{{ stats.queue_pending }}</div><div class="stat-label">Queue Pending</div></div>
    <div class="stat-card purple"><div class="stat-icon">💰</div><div class="stat-value">{{ stats.total_ads }}</div><div class="stat-label">Ads Watched</div></div>
  </div>

  <h3 class="section-title">🏆 Top Users (Today)</h3>
  <table class="user-table" style="margin-bottom:30px">
    <tr><th>#</th><th>User</th><th>Total Edits</th><th>Status</th></tr>
    {% for u in top_users %}
    <tr>
      <td style="color:#aaa">{{ loop.index }}</td>
      <td>{{ u.first_name or 'Unknown' }}{% if u.username %} <span style="color:#aaa">@{{ u.username }}</span>{% endif %}</td>
      <td><strong style="color:#4facfe">{{ u.total_edits }}</strong></td>
      <td>{% if u.is_premium %}<span class="badge-prem">⭐ Premium</span>{% elif u.is_banned %}<span class="badge-ban">🚫 Banned</span>{% else %}<span class="badge-free">Free</span>{% endif %}</td>
    </tr>
    {% else %}
    <tr><td colspan="4" style="text-align:center;color:#aaa;padding:20px">No users yet</td></tr>
    {% endfor %}
  </table>
</div>
...
<div class="tab-content" id="tab-status">
  ...
  <table class="info-table" style="margin-bottom:25px">
    <tr><td>Bot Status</td><td><span class="status-dot"></span>Online &amp; Running</td></tr>
    ...
    <tr><td>Database Size</td><td>{{ db_size_kb }} KB</td></tr>
  </table>
  <div class="alert">✅ All core services are operating normally. Bot is connected to Telegram and processing requests.</div>
</div>
...
<div class="tab-content" id="tab-users">
  ...
  <table class="user-table">
    <tr><th>ID</th><th>Name</th><th>Edits</th><th>Renames</th><th>Status</th><th>Joined</th></tr>
    {% for u in recent_users %}
    <tr>
      <td style="color:#aaa;font-family:monospace">{{ u.user_id }}</td>
      <td>{{ u.first_name or 'Unknown' }}{% if u.username %} <span style="color:#aaa">@{{ u.username }}</span>{% endif %}</td>
      <td><strong style="color:#4facfe">{{ u.total_edits }}</strong></td>
      <td>{{ u.renamed_files }}</td>
      <td>{% if u.is_premium %}<span class="badge-prem">⭐ Premium</span>{% elif u.is_banned %}<span class="badge-ban">🚫 Banned</span>{% else %}<span class="badge-free">Free</span>{% endif %}</td>
      <td style="color:#aaa;font-size:0.82em">{{ u.joined_date or 'N/A' }}</td>
    </tr>
    {% else %}
    <tr><td colspan="6" style="text-align:center;color:#aaa;padding:20px">No users yet</td></tr>
    {% endfor %}
  </table>
</div>
...
<div class="tab-content" id="tab-tools">
  <h3 class="section-title">🛠️ Available Tools ({{ total_tools }}+)</h3>
  <div class="tools-grid" style="margin-bottom:30px">
    <div class="tool-badge">📸 66+ Image Filters</div>
    <div class="tool-badge">🎬 77+ Video Effects</div>
    <div class="tool-badge">🤖 AI Tools</div>
    <div class="tool-badge">🗜️ 144p–4K Export</div>
    <div class="tool-badge">📝 File Rename</div>
    <div class="tool-badge">🏷️ Metadata Edit</div>
    <div class="tool-badge">🎨 Logo Maker</div>
    <div class="tool-badge">🗣️ Text-to-Speech</div>
    <div class="tool-badge">🖼️ Text-to-Image</div>
    <div class="tool-badge">🎥 Text-to-Video</div>
    <div class="tool-badge">🧠 AI Prompt Edit</div>
    ...
    <div class="tool-badge">💬 Caption Edit</div>
  </div>
  <h3 class="section-title">📸 Image Filter Categories</h3>
  <div class="tools-grid">
    {% for cat in image_cats %}
    <div class="tool-badge" style="border-color:rgba(240,147,251,0.4);color:#f093fb">{{ cat }}</div>
    {% endfor %}
  </div>
</div>
...
<div class="tab-content" id="tab-daily">
  <h3 class="section-title">📈 Daily Edits (Last 7 Days)</h3>
  <div class="bar-chart">
    {% for date, count in daily_stats %}
    <div class="bar-row">
      <div class="bar-label">{{ date }}</div>
      <div class="bar-track"><div class="bar-fill" style="width:{{ [count * 2, 100]|min }}%"></div></div>
...
<div class="tab-content" id="tab-admin">
  <h3 class="section-title">🔧 Quick Admin Info</h3>
  <div class="action-grid" style="margin-bottom:25px">
    <div class="action-card"><div class="icon">👑</div><div class="name">Owner ID</div><div class="desc" style="font-family:monospace;color:#4facfe">{{ owner_id }}</div></div>
    <div class="action-card"><div class="icon">📊</div><div class="name">DB Tables</div><div class="desc">{{ db_tables }} tables tracked</div></div>
    <div class="action-card"><div class="icon">💾</div><div class="name">Database</div><div class="desc">{{ db_size_kb }} KB</div></div>
    <div class="action-card"><div class="icon">🤖</div><div class="name">Bot Mode</div><div class="desc">{{ maintenance_mode }}</div></div>
    <div class="action-card"><div class="icon">📢</div><div class="name">Ads System</div><div class="desc">{{ ads_status }}</div></div>
    <div class="action-card"><div class="icon">💧</div><div class="name">Watermark</div><div class="desc">{{ watermark_status }}</div></div>
  </div>
  <h3 class="section-title">📋 Admin Commands Reference</h3>
  <table class="info-table">
    <tr><td>/ban id,reason,days</td><td>Ban a user</td></tr>
    <tr><td>/unban id</td><td>Unban a user</td></tr>
    <tr><td>/addprem id,days</td><td>Grant premium</td></tr>
    <tr><td>/rmprem id</td><td>Remove premium</td></tr>
    <tr><td>/addadmin id [title]</td><td>Add admin</td></tr>
    <tr><td>/rmadmin id</td><td>Remove admin</td></tr>
    <tr><td>/userinfo id</td><td>User info</td></tr>
    <tr><td>/broadcast msg</td><td>Broadcast message</td></tr>
    <tr><td>/addcmd cmd | response</td><td>Add custom command</td></tr>
    <tr><td>/setads on,interval</td><td>Configure ads</td></tr>
    <tr><td>/addcoins id amount</td><td>Add coins to user</td></tr>
  </table>
</div>

<div class="footer">
  🤖 KiraFx Media Editor Bot v7.0 &nbsp;|&nbsp; Made by Kira Version Team &nbsp;|&nbsp; {{ now }}
</div>
...
def dashboard():
    ...
    try:
        ...
        top_users = []
    try:
        db.cursor.execute("SELECT * FROM users ORDER BY rowid DESC LIMIT 15")
        recent_users = [dict(r) for r in db.cursor.fetchall()]
    except:
        recent_users = []
    try:
        daily_raw = db.get_daily_stats()
        daily_stats = sorted(daily_raw.items(), reverse=True)[:7]
    except:
        daily_stats = []
    try:
        db.cursor.execute("SELECT COUNT(*) FROM sqlite_master WHERE type='table'")
        db_tables = db.cursor.fetchone()[0]
        db_size_kb = f"{db.get_db_size()/1024:.1f}"
    except:
        db_tables = 0
        db_size_kb = "N/A"
    try:
        maintenance_mode = "🔒 Maintenance ON" if db.get_setting('maintenance_mode','0')=='1' else "🟢 Normal Mode"
        ads_status = "✅ Enabled" if db.get_setting('ads_enabled','1')=='1' else "❌ Disabled"
        watermark_status = "✅ Enabled" if db.get_setting('watermark_enabled','1')=='1' else "❌ Disabled"
    except:
        maintenance_mode = ads_status = watermark_status = "N/A"
    image_cats = list(MediaProcessor.IMAGE_FILTERS.keys()) if hasattr(MediaProcessor,'IMAGE_FILTERS') else []
    total_tools = (sum(len(v) for v in MediaProcessor.IMAGE_FILTERS.values()) +
                   sum(len(v) for v in MediaProcessor.VIDEO_EFFECTS.values())) if hasattr(MediaProcessor,'IMAGE_FILTERS') else 0
    return render_template_string(DASHBOARD_HTML,
        ...
        daily_stats=daily_stats, db_tables=db_tables,
        db_size_kb=db_size_kb, maintenance_mode=maintenance_mode,
        ads_status=ads_status, watermark_status=watermark_status,
        owner_id=getattr(Config,'OWNER_ID','N/A'),
        image_cats=image_cats, total_tools=total_tools)
...
class InlineKB:
    ...
    def main_menu(user_id):
        ...
        rows = [
            ...
            [InlineKeyboardButton("🖼️ AI Text→Image", callback_data="menu_txt2img"),
             InlineKeyboardButton("🎥 AI Text→Video", callback_data="menu_txt2vid")],
            [InlineKeyboardButton("🧠 AI Prompt Edit", callback_data="menu_aiedit"),
        ...
        if Config.UPDATE_CHANNEL:
            channel_row.append(InlineKeyboardButton("📢 Update Channel", url=Config.UPDATE_CHANNEL))
        else:
            channel_row.append(InlineKeyboardButton("📢 Updates", callback_data="no_channel"))
        if Config.SUPPORT_CHAT:
            channel_row.append(InlineKeyboardButton("💬 Support Group", url=Config.SUPPORT_CHAT))
        else:
            channel_row.append(InlineKeyboardButton("💬 Support", callback_data="no_support"))
    ...
    async def get_video_metadata(video_path: str) -> dict:
        try:
            ...
            return {
                ...
                'fps': (lambda s: float(s.split('/')[0])/float(s.split('/')[1]) if '/' in s and float(s.split('/')[1]) != 0 else float(s) if s else 0)(vid.get('r_frame_rate', '0')),
...
def generate_text_to_image(prompt: str, width: int = 800, height: int = 600,
                            style: str = "gradient") -> str:
    """Generate a styled image from text using PIL."""
    import textwrap, math
    os.makedirs(Config.TEMP_DIR, exist_ok=True)
    out = tempfile.NamedTemporaryFile(suffix=".png", delete=False, dir=Config.TEMP_DIR).name

    style_configs = {
        "gradient": {"bg": [(15, 12, 41), (48, 43, 99), (36, 36, 62)], "text": (255, 255, 255), "accent": (79, 172, 254)},
        "neon":     {"bg": [(0, 0, 0), (0, 20, 40), (0, 0, 0)], "text": (0, 255, 200), "accent": (255, 0, 150)},
        "gold":     {"bg": [(20, 10, 0), (40, 25, 0), (20, 10, 0)], "text": (255, 215, 0), "accent": (255, 165, 0)},
        "dark":     {"bg": [(10, 10, 10), (25, 25, 35), (10, 10, 10)], "text": (230, 230, 255), "accent": (120, 80, 255)},
        "fire":     {"bg": [(20, 5, 0), (40, 15, 0), (60, 10, 0)], "text": (255, 200, 50), "accent": (255, 80, 0)},
    }
    cfg = style_configs.get(style, style_configs["gradient"])
    ...
    for y in range(height):
        t = y / height
        if t < 0.5:
            r = int(cfg["bg"][0][0] + (cfg["bg"][1][0] - cfg["bg"][0][0]) * t * 2)
            g = int(cfg["bg"][0][1] + (cfg["bg"][1][1] - cfg["bg"][0][1]) * t * 2)
            b = int(cfg["bg"][0][2] + (cfg["bg"][1][2] - cfg["bg"][0][2]) * t * 2)
        else:
            r = int(cfg["bg"][1][0] + (cfg["bg"][2][0] - cfg["bg"][1][0]) * (t - 0.5) * 2)
            g = int(cfg["bg"][1][1] + (cfg["bg"][2][1] - cfg["bg"][1][1]) * (t - 0.5) * 2)
            b = int(cfg["bg"][1][2] + (cfg["bg"][2][2] - cfg["bg"][1][2]) * (t - 0.5) * 2)
    ...
    for i in range(20):
        ...
        ac = cfg["accent"]
        draw.ellipse([cx-r2, cy-r2, cx+r2, cy+r2], fill=(ac[0], ac[1], ac[2]))
    ...
    words = textwrap.wrap(prompt, width=32)
    total_h = len(words) * 52
    start_y = (height - total_h) // 2 - 20

    for i, line in enumerate(words):
        x = width // 2
        y = start_y + i * 52
        draw.text((x+2, y+2), line, font=font_title, fill=(0, 0, 0, 120), anchor="mm")
        draw.text((x, y), line, font=font_title, fill=cfg["text"], anchor="mm")

    sub_text = f"✨ KiraFx AI Text-to-Image"
    draw.text((width//2, height - 60), sub_text, font=font_sub, fill=cfg["accent"], anchor="mm")
    brand = "Made by Kira Version Team"
    draw.text((width//2, height - 30), brand, font=font_brand, fill=(150, 150, 180), anchor="mm")

    img.save(out, "PNG", quality=95)
    return out


def generate_text_to_video(prompt: str, duration: int = 5, style: str = "gradient") -> str:
    """Generate a short video from text using PIL + ffmpeg."""
    if not shutil.which('ffmpeg'):
        raise RuntimeError("FFmpeg not available")
    os.makedirs(Config.TEMP_DIR, exist_ok=True)
    frames_dir = tempfile.mkdtemp(dir=Config.TEMP_DIR)
    out_path = tempfile.NamedTemporaryFile(suffix=".mp4", delete=False, dir=Config.TEMP_DIR).name
    fps = 24
    ...
    import textwrap as _tw, math
    wrapped = _tw.wrap(prompt, width=28)
    bg_colors = [(15,12,41),(48,43,99),(75,60,130),(48,43,99),(15,12,41)]

    for f in range(total_frames):
        t = f / total_frames
        idx = int(t * (len(bg_colors) - 1))
        c0, c1 = bg_colors[idx], bg_colors[min(idx+1, len(bg_colors)-1)]
        ft = (t * (len(bg_colors) - 1)) - idx
        bg = tuple(int(c0[i] + (c1[i]-c0[i]) * ft) for i in range(3))
        ...
        for i, line in enumerate(wrapped):
            y = 200 + i * 56 - (len(wrapped) * 28)
            pulse = int(4 * abs(math.sin(f * 0.05 + i * 0.5)))
            draw.text((427+2, y+2), line, font=font_title, fill=(0, 0, 0))
            draw.text((427, y), line, font=font_title, fill=(255, 255, 255))

        prog = "▓" * int(t * 30) + "░" * (30 - int(t * 30))
        draw.text((427, 430), f"🎬 {prog}", font=font_sub, fill=(79, 172, 254), anchor="mm")
        draw.text((427, 460), "Made by Kira Version Team", font=font_sub, fill=(100, 100, 160), anchor="mm")
    ...
    _sp.run([
        'ffmpeg', '-r', str(fps), '-i', os.path.join(frames_dir, 'frame_%05d.png'),
...
def parse_prompt_to_image_filter(prompt: str) -> str:
    """Map a natural language prompt to an image filter name."""
    prompt_lower = prompt.lower()
    mappings = [
        (["vintage", "retro", "old", "classic", "film"], "vintage"),
        (["black and white", "bw", "grayscale", "mono", "noir"], "grayscale"),
        (["bright", "lighten", "brighter", "illuminate"], "brighten"),
        (["dark", "darker", "shadow", "dim"], "darken"),
        (["warm", "sunset", "orange", "golden"], "warm"),
        (["cool", "cold", "blue", "winter", "icy"], "cool"),
        (["cartoon", "anime", "illustration", "drawing"], "cartoon"),
        (["sketch", "pencil", "hand drawn", "outline"], "pencil_sketch"),
        (["sepia", "brown", "old photo"], "sepia"),
        (["blur", "soft", "dreamy", "hazy"], "blur"),
        (["sharp", "crisp", "detail", "enhance"], "sharpen"),
        (["neon", "glow", "electric", "vivid"], "neon_glow"),
        (["dramatic", "cinema", "cinematic", "movie"], "cinematic"),
        (["art", "paint", "oil", "watercolor"], "oil_paint"),
        (["pixel", "8bit", "retro game", "mosaic"], "pixelate"),
        (["mirror", "reflect", "flip"], "mirror"),
        (["comic", "manga"], "comic"),
        (["edge", "outline", "contour"], "edge_detect"),
        (["poster", "pop art", "colorful"], "pop_art"),
        (["emboss", "relief", "3d effect"], "emboss"),
        (["contrast", "vibrant", "punchy"], "enhance_contrast"),
        (["hue", "color shift", "rainbow"], "hue_rotate_90"),
        (["vignette", "frame", "border"], "vignette"),
        (["invert", "negative", "opposite"], "invert"),
        (["fade", "muted", "desaturate", "washed"], "fade"),
        (["glitch", "error", "digital"], "glitch"),
        (["bokeh", "depth", "background blur"], "lens_blur"),
        (["hdr", "vivid", "punchy", "high dynamic"], "hdr"),
        (["tilt", "shift", "miniature"], "tilt_shift"),
        (["sunset", "golden hour", "amber"], "golden_hour"),
    ]
    for keywords, filter_name in mappings:
        if any(kw in prompt_lower for kw in keywords):
            return filter_name
    return "enhance_contrast"


def parse_prompt_to_video_effect(prompt: str) -> str:
    """Map a natural language prompt to a video effect name."""
    prompt_lower = prompt.lower()
    mappings = [
        (["vintage", "retro", "old", "film", "classic"], "vintage"),
        (["black and white", "bw", "grayscale", "mono"], "grayscale"),
        (["bright", "lighten", "brighter"], "brighten"),
        (["dark", "darker", "shadow", "dim"], "darken"),
        (["speed up", "fast", "faster", "2x", "timelapse"], "speed_up"),
        (["slow", "slow motion", "slowmo", "0.5x"], "slow_motion"),
        (["reverse", "backward", "rewind"], "reverse"),
        (["mute", "silent", "no audio", "no sound"], "mute"),
        (["mirror", "flip", "reflect", "horizontal"], "flip_horizontal"),
        (["vertical flip", "upside down"], "flip_vertical"),
        (["rotate", "turn", "90 degrees"], "rotate_90"),
        (["blur", "soft", "dream"], "blur"),
        (["sharpen", "sharp", "crisp"], "sharpen"),
        (["neon", "glow", "electric"], "neon_glow"),
        (["cinematic", "movie", "film", "widescreen"], "cinematic_bars"),
        (["sepia", "brown", "old photo"], "sepia"),
        (["cartoon", "animation", "anime"], "cartoon"),
        (["vignette", "frame", "border"], "vignette"),
        (["contrast", "vibrant", "hdr", "punchy"], "enhance_contrast"),
        (["warm", "orange", "golden", "sunset"], "warm"),
        (["cool", "cold", "blue", "winter"], "cool"),
        (["glitch", "error", "digital", "distort"], "glitch"),
        (["zoom", "scale", "crop"], "zoom_crop"),
        (["extract audio", "audio only", "mp3", "sound"], "extract_audio"),
        (["thumbnail", "screenshot", "frame", "snapshot"], "thumbnail"),
    ]
    for keywords, effect_name in mappings:
        if any(kw in prompt_lower for kw in keywords):
            return effect_name
    return "enhance_contrast"
...
class MediaEditorBot:
    ...
    def register_handlers(self):
        ...
        @self.app.on_message(filters.command(["txt2img", "texttoimage", "aiimage"]) & filters.private)
        async def txt2img_cmd(client, message):
            uid = message.from_user.id
            db.set_session(uid, "awaiting_txt2img_prompt", {"style": "gradient"})
            await message.reply(
                "<b>🖼️ KiraFx AI Text → Image</b>\n\n"
                "Send me your <b>prompt/text</b> and I will generate a beautiful styled image!\n\n"
                "Examples:\n"
                "• <i>A beautiful sunset over mountains</i>\n"
                "• <i>KiraFx logo neon blue</i>\n"
                "• <i>Happy birthday John</i>\n\n"
                "Choose a style, then send your prompt:",
                parse_mode=enums.ParseMode.HTML,
                reply_markup=InlineKeyboardMarkup([
                    [InlineKeyboardButton("🎨 Gradient", callback_data="t2i_style:gradient"),
                     InlineKeyboardButton("⚡ Neon", callback_data="t2i_style:neon")],
                    [InlineKeyboardButton("🌟 Gold", callback_data="t2i_style:gold"),
                     InlineKeyboardButton("🌙 Dark", callback_data="t2i_style:dark")],
                    [InlineKeyboardButton("🔥 Fire", callback_data="t2i_style:fire"),
                     InlineKeyboardButton("🔙 Menu", callback_data="main_menu")],
                ])
            )

        @self.app.on_message(filters.command(["txt2vid", "texttovideo", "aivideo"]) & filters.private)
        async def txt2vid_cmd(client, message):
            uid = message.from_user.id
            db.set_session(uid, "awaiting_txt2vid_prompt", {"style": "gradient", "duration": 5})
            await message.reply(
                "<b>🎥 KiraFx AI Text → Video</b>\n\n"
                "Send me a <b>prompt/text</b> and I will generate an animated video!\n\n"
                "Examples:\n"
                "• <i>KiraFx Media Bot promo video</i>\n"
                "• <i>Summer vibes 2024</i>\n\n"
                "Choose duration, then send your prompt:",
                parse_mode=enums.ParseMode.HTML,
                reply_markup=InlineKeyboardMarkup([
                    [InlineKeyboardButton("⏱ 3 sec", callback_data="t2v_dur:3"),
                     InlineKeyboardButton("⏱ 5 sec", callback_data="t2v_dur:5"),
                     InlineKeyboardButton("⏱ 10 sec", callback_data="t2v_dur:10")],
                    [InlineKeyboardButton("🔙 Menu", callback_data="main_menu")],
                ])
            )

        @self.app.on_message(filters.command(["aiedit", "promptedit", "ai"]) & filters.private)
        async def aiedit_cmd(client, message):
            uid = message.from_user.id
            session, step, _ = db.get_session(uid)
            if session and 'file_id' in session:
                sess_type = step or ''
                if 'video' in sess_type:
                    db.set_session(uid, "awaiting_ai_vid_prompt", session)
                    prompt_type = "video effect"
                else:
                    db.set_session(uid, "awaiting_ai_img_prompt", session)
                    prompt_type = "image filter"
                await message.reply(
                    f"<b>🧠 KiraFx AI Prompt Editor</b>\n\n"
                    f"Describe how you want to edit your {prompt_type}!\n\n"
                    f"Examples:\n"
                    f"• <i>make it look vintage and warm</i>\n"
                    f"• <i>add a cinematic dark effect</i>\n"
                    f"• <i>make it black and white with high contrast</i>\n"
                    f"• <i>add neon glow effect</i>\n\n"
                    f"Just type your prompt:",
                    parse_mode=enums.ParseMode.HTML,
                    reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 Menu", callback_data="main_menu")]])
                )
            else:
                await message.reply(
                    "<b>🧠 KiraFx AI Prompt Editor</b>\n\n"
                    "First send me a <b>photo or video</b>, then use this to apply AI-powered edits by describing what you want!\n\n"
                    "Or try:\n• /txt2img — Generate image from text\n• /txt2vid — Generate video from text",
                    parse_mode=enums.ParseMode.HTML,
                    reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 Menu", callback_data="main_menu")]])
                )
...
<i>Made by Kira Version Team</i>

<b>📸 Image Tools (66+):</b>
• Basic: Blur, Sharpen, Grayscale, Sepia, Edge Enhance, Emboss, Smooth, Detail
• Artistic: Oil Paint, Watercolor, Sketch, Cartoon, Glitch, Vintage, Neon, Mosaic, Comic, Pastel
• Color: Brightness, Contrast, Saturation, Hue, Temperature, Auto Color, Equalize, Posterize, Invert
• Transform: Rotate, Flip, Resize, Crop, Skew, Mirror
• Special: Blur BG, Vignette, Border, Rounded, Shadow, Glow, Duotone, Sunset, Winter, Autumn

<b>🎥 Video Tools (77+):</b>
• Basic: Trim, Cut, Speed (0.25x-4x), Reverse, Loop, Merge, GIF
• Filters: B&amp;W, Sepia, Vintage, Cinematic, Glitch, Pixelate, VHS, RGB Shift
• Audio: Mute, Extract, Add Music, Volume, Bass, Echo, Reverb
• Speed: Speed Up 2x, Slow Motion 0.5x, Timelapse
• AI: Speech, Subtitles, Face Blur, Scene Detect

<b>🤖 AI Generation (NEW):</b>
/txt2img - Generate styled image from text prompt
/txt2vid - Generate animated video from text prompt
/aiedit - AI prompt-based photo/video editing
/ai - AI tools menu

<b>🗜️ Compression (144p to 4K):</b>
/compress - Choose quality preset

<b>📝 Advanced Rename + Metadata:</b>
/rename - Rename file (up to 4GB) + thumbnail
/metadata - Edit title, artist, album, year...

<b>🎨 Logo Maker:</b>
/logo - Custom logos (gradient · gold · neon · fire · ice)

<b>📝 All Commands:</b>
/start /help /premium /trial /status /profile
/refer /rename /metadata /compress /logo
/txt2img /txt2vid /aiedit
/ping /alive /queue

<b>📢 Admin Commands:</b>
/admin - Admin panel (60+ tools)
/ban /unban /addprem /rmprem /broadcast /addcmd
...
<b>Trial Used:</b> {'Yes' if row.get('trial_used') else 'No'}
    ...
    async def process_image(self, client, message):
        ...
        edit_buttons = InlineKeyboardMarkup([
            ...
            [InlineKeyboardButton("🧠 AI Prompt Edit", callback_data="ai_img_prompt"),
             InlineKeyboardButton("🔀 Before/After", callback_data="compare_last_img")],
            [InlineKeyboardButton("🗜️ Compress", callback_data="menu_compress"),
             InlineKeyboardButton("📝 Rename/Upload", callback_data="rename_start")],
            [InlineKeyboardButton("🎨 Logo Maker", callback_data="menu_logo"),
             InlineKeyboardButton("🖼️ Text→Image", callback_data="menu_txt2img")],
            [InlineKeyboardButton("🏠 Main Menu", callback_data="main_menu")]
        ])
        await message.reply(
            f"<b>📸 Image Ready! ✅</b>\n\nSpeed: {speed}\n"
            + (f"Caption: <i>{html_module.escape(caption[:60])}</i>\n" if caption else "")
            + "\n<b>66+ filters</b> · AI prompt editing · logo maker — choose:",
            parse_mode=enums.ParseMode.HTML,
    ...
    async def process_video(self, client, message):
        ...
        video_buttons = InlineKeyboardMarkup([
            [InlineKeyboardButton("🎨 Apply Effect (77+)", callback_data="vid_choose_effect"),
             InlineKeyboardButton("🧠 AI Prompt Edit", callback_data="ai_vid_prompt")],
            ...
            [InlineKeyboardButton("⚡ Speed Up 2x", callback_data="apply_vid:speed_up"),
             InlineKeyboardButton("🐢 Slow Motion", callback_data="apply_vid:slow_motion")],
            [InlineKeyboardButton("🖼️ Extract Thumbnail", callback_data="thumb_extract"),
             InlineKeyboardButton("🔁 Reverse Video", callback_data="apply_vid:reverse")],
            [InlineKeyboardButton("🎥 Text→Video", callback_data="menu_txt2vid"),
             InlineKeyboardButton("🏠 Main Menu", callback_data="main_menu")]
        ])
        await message.reply(
            f"<b>🎬 Video Ready! ✅</b>\n\n"
            ...
            + "\n<b>77+ effects</b> · AI prompt editing · trim/speed/reverse — choose:",
            parse_mode=enums.ParseMode.HTML,
    ...
    async def handle_text(self, client, message):
        ...
        # ---- AI Text-to-Image ----
        if step == "awaiting_txt2img_prompt":
            prompt_text = text.strip()[:200]
            style = (session or {}).get('style', 'gradient')
            if not prompt_text:
                await message.reply("❌ Please send a non-empty prompt.")
                return
            pm = await message.reply(f"⏳ Generating image for: <i>{html_module.escape(prompt_text[:60])}</i>...", parse_mode=enums.ParseMode.HTML)
            try:
                img_path = await asyncio.get_event_loop().run_in_executor(
                    None, lambda: generate_text_to_image(prompt_text, style=style))
                await client.send_photo(
                    uid, img_path,
                    caption=(
                        f"<b>🖼️ KiraFx AI Text → Image</b>\n\n"
                        f"📝 Prompt: <i>{html_module.escape(prompt_text[:100])}</i>\n"
                        f"🎨 Style: {style}\n\n"
                        f"<i>Made by Kira Version Team</i>"
                    ),
                    parse_mode=enums.ParseMode.HTML,
                    reply_markup=InlineKeyboardMarkup([
                        [InlineKeyboardButton("🔄 Generate Again", callback_data="menu_txt2img"),
                         InlineKeyboardButton("🎨 Change Style", callback_data="menu_txt2img")],
                        [InlineKeyboardButton("🏠 Main Menu", callback_data="main_menu")]
                    ])
                )
                ...
                await pm.edit_text(f"❌ Generation failed: {str(e)[:200]}")
            return

        # ---- AI Text-to-Video ----
        if step == "awaiting_txt2vid_prompt":
            prompt_text = text.strip()[:200]
            style = (session or {}).get('style', 'gradient')
            duration = int((session or {}).get('duration', 5))
            if not prompt_text:
                await message.reply("❌ Please send a non-empty prompt.")
                return
            pm = await message.reply(
                f"⏳ Generating {duration}s video for: <i>{html_module.escape(prompt_text[:60])}</i>...\n\nThis may take up to 30 seconds...",
                parse_mode=enums.ParseMode.HTML
            )
            try:
                vid_path = await asyncio.get_event_loop().run_in_executor(
                    None, lambda: generate_text_to_video(prompt_text, duration=duration, style=style))
                await client.send_video(
                    uid, vid_path,
                    caption=(
                        f"<b>🎥 KiraFx AI Text → Video</b>\n\n"
                        f"📝 Prompt: <i>{html_module.escape(prompt_text[:100])}</i>\n"
                    ...
                    reply_markup=InlineKeyboardMarkup([
                        [InlineKeyboardButton("🔄 Generate Again", callback_data="menu_txt2vid"),
                         InlineKeyboardButton("🏠 Main Menu", callback_data="main_menu")]
                    ])
                )
                try: os.unlink(vid_path)
                ...
                await pm.edit_text(f"❌ Video generation failed: {str(e)[:200]}")
            return

        # ---- AI Image Prompt Edit ----
        if step == "awaiting_ai_img_prompt":
            prompt_text = text.strip()
            file_id = (session or {}).get('file_id')
            if not file_id:
                await message.reply("❌ Image session expired. Please re-send the image.")
                db.clear_session(uid)
                return
            filter_name = parse_prompt_to_image_filter(prompt_text)
            filter_title = filter_name.replace('_', ' ').title()
            db.set_session(uid, 'image_edit', session)
            await message.reply(
                f"<b>🧠 AI matched your prompt to:</b>\n\n"
                f"🎨 Filter: <b>{filter_title}</b>\n"
                f"📝 Prompt: <i>{html_module.escape(prompt_text[:100])}</i>\n\n"
                f"Applying now...",
                parse_mode=enums.ParseMode.HTML
            )
            ...
            class FakeCallback:
                ...
                class message:
                    pass
            fc = FakeCallback()
            await self.apply_image_edit(client, fc, filter_name)
            db.clear_session(uid)
            return

        # ---- AI Video Prompt Edit ----
        if step == "awaiting_ai_vid_prompt":
            prompt_text = text.strip()
            file_id = (session or {}).get('file_id')
            if not file_id:
                await message.reply("❌ Video session expired. Please re-send the video.")
                db.clear_session(uid)
                return
            effect_name = parse_prompt_to_video_effect(prompt_text)
            effect_title = effect_name.replace('_', ' ').title()
            db.set_session(uid, 'video_edit', session)
            await message.reply(
                f"<b>🧠 AI matched your prompt to:</b>\n\n"
                f"🎬 Effect: <b>{effect_title}</b>\n"
                f"📝 Prompt: <i>{html_module.escape(prompt_text[:100])}</i>\n\n"
                f"Applying now...",
                parse_mode=enums.ParseMode.HTML
            )
            class FakeCallback2:
                ...
                class message:
                    pass
            fc2 = FakeCallback2()
            await self.apply_video_edit(client, fc2, effect_name)
            db.clear_session(uid)
            return
    ...
    async def handle_callback(self, client, callback):
        ...
        # ---- TEXT-TO-IMAGE ----
        elif data == "menu_txt2img":
            session2, step2, _ = db.get_session(uid)
            curr_style = (session2 or {}).get('style', 'gradient') if step2 == 'awaiting_txt2img_prompt' else 'gradient'
            db.set_session(uid, "awaiting_txt2img_prompt", {"style": curr_style})
            await callback.message.edit_text(
                "<b>🖼️ KiraFx AI Text → Image</b>\n\n"
                "Send me any text or prompt and I'll generate a beautiful styled image!\n\n"
                "<b>Examples:</b>\n"
                "• <i>A magical forest at sunset</i>\n"
                "• <i>Happy Birthday Sarah!</i>\n"
                "• <i>KiraFx Premium Edition</i>\n\n"
                f"Current style: <b>{curr_style}</b>\n\n"
                "Pick a style then send your text:",
                parse_mode=enums.ParseMode.HTML,
                reply_markup=InlineKeyboardMarkup([
                    [InlineKeyboardButton("🎨 Gradient", callback_data="t2i_style:gradient"),
                     InlineKeyboardButton("⚡ Neon", callback_data="t2i_style:neon")],
        ...
        elif data.startswith("t2i_style:"):
            style = data.split(":", 1)[1]
            session2, step2, _ = db.get_session(uid)
            sess = session2 or {}
            sess['style'] = style
            db.set_session(uid, "awaiting_txt2img_prompt", sess)
            await callback.answer(f"✅ Style set to: {style.title()}", show_alert=False)

        # ---- TEXT-TO-VIDEO ----
        elif data == "menu_txt2vid":
            session2, step2, _ = db.get_session(uid)
            curr_dur = (session2 or {}).get('duration', 5) if step2 == 'awaiting_txt2vid_prompt' else 5
            db.set_session(uid, "awaiting_txt2vid_prompt", {"style": "gradient", "duration": curr_dur})
            await callback.message.edit_text(
                "<b>🎥 KiraFx AI Text → Video</b>\n\n"
                "Send me a prompt and I'll generate an animated video with text effects!\n\n"
                "<b>Examples:</b>\n"
                "• <i>Summer 2024 highlights reel</i>\n"
                "• <i>KiraFx Bot — Promo</i>\n"
                "• <i>Happy New Year animation</i>\n\n"
                f"Duration: <b>{curr_dur}s</b> — pick duration then send prompt:",
                parse_mode=enums.ParseMode.HTML,
                reply_markup=InlineKeyboardMarkup([
                    [InlineKeyboardButton("⏱ 3 sec", callback_data="t2v_dur:3"),
                     InlineKeyboardButton("⏱ 5 sec", callback_data="t2v_dur:5"),
        ...
        elif data.startswith("t2v_dur:"):
            dur = data.split(":", 1)[1]
            session2, step2, _ = db.get_session(uid)
            sess = session2 or {}
            sess['duration'] = int(dur)
            db.set_session(uid, "awaiting_txt2vid_prompt", sess)
            await callback.answer(f"✅ Duration set to {dur}s", show_alert=False)

        # ---- AI PROMPT EDIT ----
        elif data == "menu_aiedit":
            session2, step2, _ = db.get_session(uid)
            if session2 and 'file_id' in session2:
                if 'video' in (step2 or ''):
                    db.set_session(uid, "awaiting_ai_vid_prompt", session2)
                    media_type = "video"
                else:
                    db.set_session(uid, "awaiting_ai_img_prompt", session2)
                    media_type = "image"
                await callback.message.edit_text(
                    f"<b>🧠 KiraFx AI Prompt Editor</b>\n\n"
                    f"Describe what you want to do with your {media_type}!\n\n"
                    f"<b>Examples:</b>\n"
                    f"• <i>make it look vintage and warm</i>\n"
                    f"• <i>add cinematic black bars and darken</i>\n"
                    f"• <i>black and white high contrast</i>\n"
                    f"• <i>add neon glow effect</i>\n"
                    f"• <i>speed it up 2x</i> (for video)\n\n"
                    f"Just type your prompt below:",
                    parse_mode=enums.ParseMode.HTML,
                    reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 Menu", callback_data="main_menu")]])
                )
            else:
                await callback.message.edit_text(
                    "<b>🧠 KiraFx AI Prompt Editor</b>\n\n"
                    "First send me a <b>photo or video</b>, then tap AI Prompt Edit to describe the effect you want!\n\n"
                    "Or try:\n• 🖼️ /txt2img — Generate image from text\n• 🎥 /txt2vid — Generate video from text",
                    parse_mode=enums.ParseMode.HTML,
                    reply_markup=InlineKeyboardMarkup([
                        [InlineKeyboardButton("🖼️ Text→Image", callback_data="menu_txt2img"),
                         InlineKeyboardButton("🎥 Text→Video", callback_data="menu_txt2vid")],
        ...
        elif data == "ai_img_prompt":
            session2, step2, _ = db.get_session(uid)
            if session2 and 'file_id' in session2:
                db.set_session(uid, "awaiting_ai_img_prompt", session2)
                await callback.message.edit_text(
                    "<b>🧠 AI Prompt Image Edit</b>\n\n"
                    "Type how you want to edit the image:\n\n"
                    "• <i>make it vintage</i>\n"
                    "• <i>add neon glow</i>\n"
                    "• <i>black and white dramatic</i>\n"
                    "• <i>warm sunset tones</i>\n"
                    "• <i>cartoon style</i>",
                    parse_mode=enums.ParseMode.HTML,
                    reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 Back", callback_data="menu_image")]])
                )
            else:
                await callback.answer("❌ Send an image first!", show_alert=True)

        elif data == "ai_vid_prompt":
            session2, step2, _ = db.get_session(uid)
            if session2 and 'file_id' in session2:
                db.set_session(uid, "awaiting_ai_vid_prompt", session2)
                await callback.message.edit_text(
                    "<b>🧠 AI Prompt Video Edit</b>\n\n"
                    "Type how you want to edit the video:\n\n"
                    "• <i>make it vintage retro</i>\n"
                    "• <i>slow motion effect</i>\n"
                    "• <i>speed it up 2x</i>\n"
                    "• <i>reverse the video</i>\n"
                    "• <i>add cinematic bars</i>",
                    parse_mode=enums.ParseMode.HTML,
                    reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 Back", callback_data="menu_video")]])
                )
            else:
                await callback.answer("❌ Send a video first!", show_alert=True)
            ...
            await callback.message.edit_text(
                ...
                reply_markup=InlineKeyboardMarkup([
                    ...
                    [InlineKeyboardButton("🖼️ Text → Image", callback_data="menu_txt2img"),
                     InlineKeyboardButton("🎥 Text → Video", callback_data="menu_txt2vid")],
                    [InlineKeyboardButton("🧠 AI Prompt Edit", callback_data="menu_aiedit"),
                     InlineKeyboardButton("✨ AI Video Tools", callback_data="menu_ai")],
        ...
        # ---- PREMIUM ----
            ...
            await callback.message.edit_text(
                "<b>📚 KiraFx Quick Help</b>\n\n"
                "<b>📸 Photo editing:</b>\n"
                "• Send photo → 66+ filters · AI prompt · caption edit\n\n"
                "<b>🎬 Video editing:</b>\n"
                "• Send video → 77+ effects · AI prompt · trim · speed · reverse\n\n"
                "<b>🤖 AI Tools:</b>\n"
                "• /txt2img → Generate image from text/prompt\n"
                "• /txt2vid → Generate video from text/prompt\n"
                "• /aiedit → AI prompt-based editing\n\n"
                "<b>🛠️ Tools:</b>\n"
                "• /rename → Rename file + thumbnail\n"
                "• /metadata → Edit metadata (title, artist...)\n"
                "• /compress → 144p–4K compression\n"
                "• /logo → Custom logo maker\n\n"
                "<b>💎 Premium:</b>\n"
