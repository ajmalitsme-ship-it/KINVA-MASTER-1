...
class InlineKB:
    ...
    def main_menu(user_id):
        ...
        rows = [
            [InlineKeyboardButton("🎨 Image Editor (66+)", callback_data="menu_image"),
             InlineKeyboardButton("🎬 Video Editor (77+)", callback_data="menu_video")],
            ...
            [InlineKeyboardButton("💬 Support", url=Config.SUPPORT_CHAT),
             InlineKeyboardButton("🔔 Alive", callback_data="bot_alive")],
        ]
        if is_prem:
    ...
    def admin_panel():
        return InlineKeyboardMarkup([
            ...
            [InlineKeyboardButton("🌐 Web Dashboard", url="http://localhost:5000"),
...
class MediaEditorBot:
    ...
    def register_handlers(self):
        ...
        async def logo_cmd(client, message):
            ...
            await message.reply(
                "🎨 **KiraFx Logo Maker**\n\n"
                "Send the **text** you want as your logo (max 20 chars).\n\n"
...
<b>Trial Used:</b> {'Yes' if row.get('trial_used') else 'No'}
    ...
    async def process_image(self, client, message):
        ...
        if step2 == "awaiting_thumb_photo":
            ...
            try:
                ...
                await pm3.edit_text(
                    "✅ **Thumbnail saved!**\n\nNow send the **new filename** for your file:",
                    reply_markup=InlineKeyboardMarkup([[
    ...
    async def process_document(self, client, message):
        ...
        if step == "awaiting_rename_file":
            ...
            await message.reply(
                ...
                f"Name: `{fname}`\n"
                f"Size: {size_str}\n\n"
                f"📝 Now send the **new filename** (keep the extension):\n"
                f"Example: `my_file{ext}`",
                reply_markup=InlineKeyboardMarkup([
                    [InlineKeyboardButton("📱 Upload as Doc", callback_data="upload_as_doc"),
                     InlineKeyboardButton("📹 Upload as Video", callback_data="upload_as_video")],
                    [InlineKeyboardButton("🖼️ Set Thumbnail", callback_data="thumb_set"),
    ...
    async def handle_text(self, client, message):
        ...
        if step == "awaiting_logo_text":
            ...
            try:
                ...
                await client.send_photo(
                    uid, logo_path,
                    caption=f"🎨 **Your Logo:** `{logo_text}`\n\nStyle: {style} | Background: {bg}\n\n_Made with KiraFx Bot v7.0_",
                    reply_markup=InlineKeyboardMarkup([
    ...
    async def do_rename(self, client, message, session):
        ...
        async def update_prog(step, label):
            try:
                await pm.edit_text(
                    f"{label}\n\n{ANIM[min(step, 5)]}\n\n"
                    f"📁 `{orig_name}` → `{new_name}`\n💾 {size_str}\n⚡ {speed}"
                )
            except:
                pass

        try:
            await update_prog(1, "📥 **Downloading file...**")
            ...
            await update_prog(3, "🔄 **Renaming file...**")
            ...
            await update_prog(4, "📤 **Uploading renamed file...**")
    ...
    async def handle_callback(self, client, callback):
        ...
        # ---- IMAGE MENUS ----
            ...
            if has_img:
                await callback.message.edit_text(
                    "📸 **Image Editor (66+ Filters)**\n\n✅ Image ready! Choose a filter category:",
                    reply_markup=InlineKB.image_categories()
                )
            else:
                await callback.message.edit_text(
                    "📸 **Image Editor (66+ Filters)**\n\n"
                    "📤 Send me a **photo** and I'll show all filters!\n\n"
        ...
        # ---- LOGO MAKER ----
            ...
            await callback.message.edit_text(
                "🎨 **KiraFx Logo Maker**\n\n"
                "Send the **text** for your logo (max 20 chars).\n\n"
...
/setads off - Disable all ads"""
    ...
    async def apply_image_edit(self, client, callback, filter_name):
        ...
        try:
            ...
            result_caption = (
                f"✅ **{filter_name.replace('_', ' ').title()}** applied!\n\n"
                f"⏱️ {proc_time:.1f}s | {speed_text}"
                + (f"\n\n💬 _{orig_caption}_" if orig_caption else "")
            )
            await client.send_photo(
                uid, output_path,
    ...
    async def apply_video_edit(self, client, callback, effect_name):
        ...
        try:
            ...
            orig_caption = session_data.get('caption', '')
            cap = (
                f"✅ **{effect_name.replace('_', ' ').title()}** applied!\n\n"
                f"⏱️ {proc_time:.1f}s | {speed_text}\n"
