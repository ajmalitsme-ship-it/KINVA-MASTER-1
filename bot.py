...
def apply_image_filter(img: Image.Image, filter_name: str) -> Image.Image:
    ...
    # Premium filters
        ...
        g = g.point(lambda x: max(x * 0.8, 0))
        b = b.point(lambda x: min(x * 1.4, 255))
        return Image.merge("RGB", (r, g, b))
...
async def cmd_status(client, message: Message):
    ...
    admin_str = f" | 👑 {row.get('admin_title', 'Admin')}" if is_admin(user_id) else ""
...
async def cmd_profile(client, message: Message):
    ...
    try:
        ...
        card.save(bio, "PNG")
        bio.seek(0)
        prem_plan = row.get("premium_plan", "N/A") or "N/A"
        prem_until = str(row.get("premium_until", "N/A") or "N/A")[:10]
        await client.send_photo(
            message.chat.id,
            photo=bio,
            caption=(
...
async def cmd_userinfo(client, message: Message):
    ...
    await message.reply(
        ...
        f"**Name:** {row['first_name']} {row['last_name'] or ''}\n"
        f"**Username:** @{row['username'] or 'N/A'}\n"
        f"**Premium:** {prem_text} (until {str(row.get('premium_until','N/A') or 'N/A')[:10]})\n"
        f"**Admin:** {admin_text} (Level {row.get('admin_level', 0)})\n"
        f"**Banned:** {banned_text}\n"
        f"**Reason:** {row.get('ban_reason', 'N/A') or 'N/A'}\n"
...
async def cmd_runcode(client, message: Message):
    ...
    if user_id != OWNER_ID:
        ...
        if not perms or not perms.get("can_run_code", 0):
...
async def cmd_info(client, message: Message):
    await message.reply(
        ...
        f"**Image Filters:** {len(IMAGE_FILTERS)}+\n"
        f"**Video Effects:** {len(VIDEO_EFFECTS)}+\n"
        f"**Premium Filters:** {len(PREMIUM_IMAGE_FILTERS)}\n"
    ...
    # ── Timeline Editor ───────────────────────────────────────
        ...
        if media_type == "image":
            ...
            for i, step in enumerate(steps):
                ...
                loop = asyncio.get_event_loop()
                ok, err = await loop.run_in_executor(
                    executor, lambda s=step["step"]: apply_video_effect(current_path, s, out_path)
                )
                if ok and os.path.exists(out_path) and os.path.getsize(out_path) > 0:
